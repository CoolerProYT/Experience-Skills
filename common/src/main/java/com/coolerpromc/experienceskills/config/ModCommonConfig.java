...
public class ModCommonConfig {
    ...
    public static void init(){
        ConfigBuilder builder = ConfigSpec.builder(Constants.MOD_ID, ConfigFormat.JSON5).side(ConfigSide.COMMON)
            .comment("""
                Disclaimer: Some skills from addon mods by other author might not use these config values, only report to original author if there is a bug related to built-in skills.
